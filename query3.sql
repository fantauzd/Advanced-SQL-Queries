...
SELECT 
    ...
    actor.first_name, 
    actor.last_name,
    IFNULL(sum_result.total_combined_film_length, 0) AS total_combined_film_length
FROM actor
LEFT JOIN (
    SELECT
        actor.actor_id,
        SUM(film.length) AS total_combined_film_length
    FROM actor
...
AS sum_result ON actor.actor_id = sum_result.actor_id
